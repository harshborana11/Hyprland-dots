...
terminalscheme="$HOME/.config/ags1/scripts/templates/terminal/scheme-base.json"
...
if [ ! -f $colormodefile ]; then
  ...
  if [ "$materialscheme" = "monochrome" ]; then
    terminalscheme="$HOME/.config/ags1/scripts/templates/terminal/scheme-monochrome.json"
  fi
fi
...
cd "$HOME/.config/ags1/scripts/" || exit
